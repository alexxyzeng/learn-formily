...
if  [ ! -n "$UMI_ENV" ] ;then
    output="learn-formily-zip-dist"
else
    output="${UMI_ENV}-learn-formily-zip-dist"
fi

# 删除之前打包好的项目

rm -rf ./*-zip-dist ./dist ./learn-formily*.zip
mkdir ./${output}
mkdir ./${output}/bin
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/learn-formily/'${UMI_ENV}'_learn-formily/g' ./${output}/package.json
  else 
    sed -i 's/learn-formily/'${UMI_ENV}'_learn-formily/g' ./${output}/package.json
  fi
fi
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/learn-formily/'${UMI_ENV}'_learn-formily/g' ./${output}/bin/stop.sh
  else 
    sed -i 's/learn-formily/'${UMI_ENV}'_learn-formily/g' ./${output}/bin/stop.sh
  fi
fi
